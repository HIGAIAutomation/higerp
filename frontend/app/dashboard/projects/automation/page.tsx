...
export default function AutomationProjectsPage() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const filtered = data.filter((p: Project) => p.category === 'Automation');
      ...
      console.error(err);
      setError('Failed to load Automation projects. Make sure backend service is active.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="font-sans min-h-screen pb-12">
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Automation Projects</h1>
          <p className="text-sm text-muted-foreground mt-2 font-medium">Track your custom system integration & flow automation pipelines</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.length === 0 ? (
              <div className="col-span-full text-center py-16 bg-secondary/30 rounded-[24px] border border-dashed border-border">
                ...
                <p className="text-sm text-muted-foreground font-semibold">No active Automation projects.</p>
              </div>
            ) : (
              projects.map((proj) => {
